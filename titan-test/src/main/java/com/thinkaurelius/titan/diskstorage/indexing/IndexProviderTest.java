...
public abstract class IndexProviderTest {
    ...
    private void storeTest(String... stores) throws Exception {
        ...
        Multimap<String, Object> doc1 = getDocument("Hello world", 1001, 5.2, Geoshape.point(48.0, 0.0), Geoshape.polygon(47.9,-0.1,48.1,-0.1,48.1,0.1,47.9,0.1,47.9,-0.1),Arrays.asList("1", "2", "3"), Sets.newHashSet("1", "2"), Instant.ofEpochSecond(1));
        Multimap<String, Object> doc2 = getDocument("Tomorrow is the world", 1010, 8.5, Geoshape.point(49.0, 1.0), Geoshape.line(48.9,0.9,49.1,0.9,49.1,1.1,48.9,1.1), Arrays.asList("4", "5", "6"), Sets.newHashSet("4", "5"), Instant.ofEpochSecond(2));
        Multimap<String, Object> doc3 = getDocument("Hello Bob, are you there?", -500, 10.1, Geoshape.point(47.0, 10.0), Geoshape.polygon(46.9,9.9,47.1,9.9,47.1,10.1,46.9,10.1,46.9,9.9), Arrays.asList("7", "8", "9"), Sets.newHashSet("7", "8"), Instant.ofEpochSecond(3));
        ...
        for (String store : stores) {
            ...
            result = tx.query(new IndexQuery(store, PredicateCondition.of(BOUNDARY, Geo.WITHIN, Geoshape.polygon(48.5,-0.5,47.5,-0.5,47.5,0.5,48.5,0.5,48.5,-0.5))));
            assertEquals(1, result.size());
            ...
            if (index.supports(new StandardKeyInformation(Geoshape.class, Cardinality.SINGLE, new Parameter("mapping", Mapping.PREFIX_TREE)), Geo.DISJOINT)) {
                ...
                result = tx.query(new IndexQuery(store, PredicateCondition.of(BOUNDARY, Geo.DISJOINT, Geoshape.polygon(48.5,-0.5,47.5,-0.5,47.5,0.5,48.5,0.5,48.5,-0.5))));
                assertEquals(2, result.size());
                assertEquals(ImmutableSet.of("doc2","doc3"), ImmutableSet.copyOf(result));
            }
            ...
            result = tx.query(new IndexQuery(store, PredicateCondition.of(BOUNDARY, Geo.INTERSECT, Geoshape.polygon(48,-1,49,-1,49,2,48,2,48,-1))));
            assertEquals(2, result.size());
            assertEquals(ImmutableSet.of("doc1","doc2"), ImmutableSet.copyOf(result));
...
//                printResult(tx.query(new RawQuery(store,"text:(you there Hello Bob)",NO_PARAS)));
            ...
            //Update some data
            add(store, "doc4", getDocument("I'ts all a big Bob", -100, 11.2, Geoshape.point(48.0, 8.0), Geoshape.line(47.5, 7.5, 48.5, 8.5), Arrays.asList("10", "11", "12"), Sets.newHashSet("10", "11"), Instant.ofEpochSecond(4)), true);
