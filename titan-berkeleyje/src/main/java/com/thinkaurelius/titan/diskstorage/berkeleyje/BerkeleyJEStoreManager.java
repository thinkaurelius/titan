...
public class BerkeleyJEStoreManager extends LocalStoreManager implements
		OrderedKeyValueStoreManager {

	private static final Logger log = LoggerFactory
			.getLogger(BerkeleyJEStoreManager.class);
	...
	public BerkeleyJEStoreManager(Configuration configuration)
	...
	private void initialize(int cachePercent) throws StorageException {
		try {
			...
			throw new PermanentStorageException(
					"Error during BerkeleyJE initialization: ", e);
		}
	...
	@Override
	public BerkeleyJETx beginTransaction(final StoreTxConfig config)
			throws StorageException {
		try {
			Transaction tx = null;
			if (transactional) {
			...
			throw new PermanentStorageException(
					"Could not start BerkeleyJE transaction", e);
		}
	}

	@Override
	public BerkeleyJEKeyValueStore openDatabase(String name)
			throws StorageException {
		...
		try {
			...
			BerkeleyJEKeyValueStore store = new BerkeleyJEKeyValueStore(name,
					db, this);
			stores.put(name, store);
			return store;
		} catch (DatabaseException e) {
			throw new PermanentStorageException(
					"Could not open BerkeleyJE data store", e);
		}
	}
	...
	public void close() throws StorageException {
		if (environment != null) {
			...
			try {
				// Wait just a little bit before closing so that independent
				// transaction threads can clean up.
				Thread.sleep(30);
			...
			try {
				...
				throw new PermanentStorageException(
						"Could not close BerkeleyJE database", e);
			}
		}
	...
	public void clearStorage() throws StorageException {
		if (!stores.isEmpty())
			throw new IllegalStateException(
					"Cannot delete store, since database is open: "
							+ stores.keySet().toString());
