...
/**
 * Queries for a slice of data identified by a start point (inclusive) and end
 * point (exclusive). Returns all {@link StaticBuffer}s that lie in this range
 * up to the given limit.
 * <p/>
 * If a SliceQuery is marked <i>static</i> it is expected that the result set
 * does not change.
 * 
...
public class SliceQuery extends BaseQuery implements BackendQuery<SliceQuery> {
	...
	public SliceQuery(final StaticBuffer sliceStart,
			final StaticBuffer sliceEnd, boolean isStatic) {
	...
	/**
	 * Whether the result set, if non-empty, is static, i.e. does not change
	 * anymore. This allows the query result (if non-empty) to be cached.
	 * 
	...
	@Override
	public int hashCode() {
		return new HashCodeBuilder().append(sliceStart).append(sliceEnd)
				.append(isStatic).append(getLimit()).toHashCode();
	}
	...
	public boolean equals(Object other) {
		...
		return sliceStart.equals(oth.sliceStart)
				...
				&& getLimit() == oth.getLimit() && isStatic == oth.isStatic;
	}
	...
	public boolean subsumes(SliceQuery oth) {
		...
		if (oth.getLimit() > getLimit())
			...
			return sliceStart.compareTo(oth.sliceStart) <= 0
					&& sliceEnd.compareTo(oth.sliceEnd) >= 0;
		else
			// this the result might be cutoff due to limit, the start must be
			// the same
			return sliceStart.compareTo(oth.sliceStart) == 0
					&& sliceEnd.compareTo(oth.sliceEnd) >= 0;
	}
	...
	public List<Entry> getSubset(SliceQuery otherQuery, List<Entry> otherResult) {
		...
		int pos = Collections.binarySearch(otherResult,
				StaticBufferEntry.of(sliceStart));
		if (pos < 0)
			pos = -pos - 1;
		for (; pos < otherResult.size() && result.size() < getLimit(); pos++) {
			Entry e = otherResult.get(pos);
			if (e.getColumn().compareTo(sliceEnd) < 0)
				result.add(e);
			else
				break;
		}
