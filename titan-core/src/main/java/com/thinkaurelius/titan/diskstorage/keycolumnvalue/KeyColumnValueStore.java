...
/**
 * Interface to a data store that has a BigTable like representation of its
 * data. In other words, the data store is comprised of a set of rows each of
 * which is uniquely identified by a key. Each row is composed of a column-value
 * pairs. For a given key, a subset of the column-value pairs that fall within a
 * column interval can be quickly retrieved.
 * <p/>
 * This interface provides methods for retrieving and mutating the data.
 * <p/>
 * In this generic representation keys, columns and values are represented as
 * ByteBuffers.
 * <p/>
...
public interface KeyColumnValueStore {
	...
	/**
	 * Returns true if the specified key exists in the store, i.e. there is at
	 * least one column-value pair for the key.
	 * 
	 * @param key
	 *            Key
	 * @param txh
	...
	/**
	 * Retrieves the list of entries (i.e. column-value pairs) for a specified
	 * query.
	 * 
	 * @param query
	 *            Query to get results for
	 * @param txh
	 *            Transaction
	...
	/**
	 * Retrieves the list of entries (i.e. column-value pairs) as specified by
	 * the given {@link SliceQuery} for all of the given keys together.
	 * 
	 * @param keys
	 *            List of keys
	 * @param query
	 *            Slicequery specifying matching entries
	 * @param txh
	 *            Transaction
	 * @return The result of the query for each of the given keys in the order
	 *         of the keys. That means, nth entry in the returned list is a list
	 *         that contains the entries that match the given query for the nth
	...
	/**
	 * Returns a {@link KeyIterator} over all keys that fall within the
	 * key-range specified by the given query and have one or more columns
	 * matching the column-range. Calling {@link KeyIterator#getEntries()}
	 * returns the list of all entries that match the column-range specified by
	 * the given query.
	 * <p/>
	...
	/**
	 * Returns a {@link KeyIterator} over all keys in the store that have one or
	 * more columns matching the column-range. Calling
	 * {@link KeyIterator#getEntries()} returns the list of all entries that
	 * match the column-range specified by the given query.
	 * <p/>
	 * This method is only supported by stores which do not keep keys in
	 * byte-order.
	 * 
	 * @param query
	 * @param txh
	...
	// like current getKeys if column-slice is such that it queries for vertex
	// state property

	/**
	 * Returns an array that describes the key boundaries of the locally hosted
	 * partition of this store.
	 * <p/>
	 * The array has two entries: the first marks the lower bound for the keys
	 * stored locally (inclusive) and the other marks the upper bound
	 * (exclusive).
	 * 
	 * @return An array with two entries describing the locally hosted partition
	 *         of this store.
	...
	/**
	 * Returns the name of this store. Each store has a unique name which is
	 * used to open it.
	 * 
	 * @return store name
