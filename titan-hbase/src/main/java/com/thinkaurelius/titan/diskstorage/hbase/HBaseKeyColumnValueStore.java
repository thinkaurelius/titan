...
public class HBaseKeyColumnValueStore implements KeyColumnValueStore {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(HBaseKeyColumnValueStore.class);

	private final String tableName;
	private final HTablePool pool;
	private final HBaseStoreManager storeManager;

	// When using shortened CF names, columnFamily is the shortname and
	// storeName is the longname
	...
	HBaseKeyColumnValueStore(HBaseStoreManager storeManager, HTablePool pool,
			String tableName, String columnFamily, String storeName) {
	...
	public List<Entry> getSlice(KeySliceQuery query, StoreTransaction txh)
			throws StorageException {
		List<List<Entry>> result = getHelper(Arrays.asList(query.getKey()),
				getFilter(query));
		return (result.isEmpty()) ? Collections.<Entry> emptyList() : result
				.get(0);
	}
	...
	public static Filter getFilter(SliceQuery query) {
		byte[] colStartBytes = query.getSliceEnd().length() > 0 ? query
				.getSliceStart().as(StaticBuffer.ARRAY_FACTORY) : null;
		byte[] colEndBytes = query.getSliceEnd().length() > 0 ? query
				.getSliceEnd().as(StaticBuffer.ARRAY_FACTORY) : null;

		Filter filter = new ColumnRangeFilter(colStartBytes, true, colEndBytes,
				false);
	...
	private List<List<Entry>> getHelper(List<StaticBuffer> keys,
			Filter getFilter) throws StorageException {
		List<Get> requests = new ArrayList<Get>(keys.size());
		{
			for (StaticBuffer key : keys) {
				requests.add(new Get(key.as(StaticBuffer.ARRAY_FACTORY))
						.addFamily(columnFamilyBytes).setFilter(getFilter));
			}
		}
		...
		try {
			...
			for (Result result : r) {
				...
				Map<byte[], byte[]> fmap = result
						.getFamilyMap(columnFamilyBytes);

				if (null != fmap) {
					for (Map.Entry<byte[], byte[]> ent : fmap.entrySet()) {
						entries.add(StaticBufferEntry.of(new StaticArrayBuffer(
								ent.getKey()),
								new StaticArrayBuffer(ent.getValue())));
					}
				}
	...
	public void mutate(StaticBuffer key, List<Entry> additions,
			...
			throws StorageException {
		Map<StaticBuffer, KCVMutation> mutations = ImmutableMap.of(key,
	...
	public KeyIterator getKeys(KeyRangeQuery query, StoreTransaction txh)
			throws StorageException {
		return executeKeySliceQuery(
				query.getKeyStart().as(StaticBuffer.ARRAY_FACTORY), query
						.getKeyEnd().as(StaticBuffer.ARRAY_FACTORY),
				new FilterList(FilterList.Operator.MUST_PASS_ALL), query);
	}

	@Override
	public KeyIterator getKeys(SliceQuery query, StoreTransaction txh)
			throws StorageException {
		return executeKeySliceQuery(new FilterList(
				FilterList.Operator.MUST_PASS_ALL), query);
	}

	public KeyIterator executeKeySliceQuery(FilterList filters,
	...
	public KeyIterator executeKeySliceQuery(@Nullable byte[] startKey,
			@Nullable byte[] endKey, FilterList filters,
			@Nullable SliceQuery columnSlice) throws StorageException {
		...
		try {
			return new RowIterator(pool.getTable(tableName).getScanner(
	 ...
	 * 
	 * @param cfName
	 *            The name of the ColumnFamily deletions belong to
	 * @param key
	 *            The row key
	 * @param deletions
	 *            The name of the columns to delete (a.k.a deletions)
	...
	private static Delete makeDeletionCommand(byte[] cfName, byte[] key,
		...
		for (StaticBuffer del : deletions) {
			deleteCommand.deleteColumn(cfName,
					del.as(StaticBuffer.ARRAY_FACTORY));
		}
	 ...
	 * 
	 * @param cfName
	 *            The name of the ColumnFamily modifications belong to
	 * @param key
	 *            The row key
	 * @param modifications
	 *            The entries to insert/update.
	 * @return Put command or null if additions were null or empty.
	 */
	private static Put makePutCommand(byte[] cfName, byte[] key,
			List<Entry> modifications) {
	...
	public static List<Row> makeBatch(byte[] cfName, byte[] key,
			List<Entry> additions, List<StaticBuffer> deletions) {
	...
	private class RowIterator implements KeyIterator {
		...
		public RowIterator(ResultScanner rows) {
			this.rows = Iterators.filter(rows.iterator(),
					new Predicate<Result>() {
						...
						public boolean apply(@Nullable Result result) {
							...
							try {
								StaticBuffer id = new StaticArrayBuffer(result
										.getRow());
								id.getLong(0);
		...
		public RecordIterator<Entry> getEntries() {
			...
			return new RecordIterator<Entry>() {
				private final Iterator<Map.Entry<byte[], byte[]>> kv = currentRow
						.getFamilyMap(columnFamilyBytes).entrySet().iterator();
				...
				public Entry next() {
					...
					return StaticBufferEntry.of(
							new StaticArrayBuffer(column.getKey()),
							new StaticArrayBuffer(column.getValue()));
				}
