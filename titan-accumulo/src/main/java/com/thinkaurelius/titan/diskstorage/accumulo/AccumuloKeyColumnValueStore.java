...
public class AccumuloKeyColumnValueStore implements KeyColumnValueStore {
    ...
    public boolean containsKey(StaticBuffer key, StoreTransaction txh) throws StorageException {
        ...
        try {
            Scanner scanner = connector.createScanner(tableName, DEFAULT_AUTHORIZATIONS);
    ...
    public List<Entry> getSlice(KeySliceQuery query, StoreTransaction txh) throws StorageException {
        ...
        for (Map.Entry<Key, Value> entry : scanner) {
            ...
            if (count < query.getLimit()) {
                if (startBytes.length == 0 || byteCompare(startBytes, cqBytes) <= 0) {
                    if (endBytes.length == 0 || byteCompare(cqBytes, endBytes) < 0) {
    ...
    private static int byteCompare(final byte[] left, final byte[] right) {
        return byteCompare(left, 0, left.length, right, 0, right.length);
    }

    private static int byteCompare(byte[] buffer1, int offset1, int length1,
