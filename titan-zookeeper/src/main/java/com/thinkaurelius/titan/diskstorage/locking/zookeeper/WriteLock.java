...
public class WriteLock extends ProtocolSupport {
     ...
     */
    public WriteLock(ZooKeeper zookeeper, String dir, List<ACL> acl, long expireMS) {
        super(zookeeper);
        this.dir = dir;
        if (acl != null) {
            setAcl(acl);
    ...
    public WriteLock(ZooKeeper zookeeper, String dir, List<ACL> acl, 
            long expireMS, LockListener callback) {
        this(zookeeper, dir, acl, expireMS);
            ...
            do {
                ...
                if (id != null) {
                    ...
                    if (names.isEmpty()) {
                         ...
                         * Sort the children
                         * 
                         * Although getChildren(...) sometimes returns sorted
                         * children in practice, the javodoc for
                         * getChildren(...) explicitly states that the nodes are
                         * not necessarily returned in order.
                         */
                        SortedSet<ZNodeName> sortedNames = new TreeSet<ZNodeName>();
                        for (String name : names) {
                            sortedNames.add(new ZNodeName(dir + "/" + name));
                        }
                        // We could check expiration time lazily -- that is, only if we are not at the head of the sorted set
                        sortedNames = removeExpiredNodes(sortedNames);
    ...
    private SortedSet<ZNodeName> removeExpiredNodes(SortedSet<ZNodeName> s) throws KeeperException, InterruptedException {
        long nowMS = System.currentTimeMillis();
        SortedSet<ZNodeName> result = new TreeSet<ZNodeName>();
        for (ZNodeName n : s) {
            Stat st = zookeeper.exists(n.getName(), false);
            if (expireMS <= nowMS - st.getMtime()) {
                LOG.warn("Removing expired lock node {}", n);
                zookeeper.delete(n.getName(), st.getVersion());
            } else {
                LOG.debug("Keeping unexpired lock node {}", n);
                result.add(n);
            }
        }
        return result;
